...
app.delete('/posts/:postId/comments/:commentId', async (req, res) => {
    ...
    let result = await db.collection('postcomments').updateOne(
...
app.post('/posts/:postId/comments', async (req, res) => {
    ...
    let result = await db.collection('postcomments').insertOne(doc);
    /*{ _id: mongo.ObjectId(postId) },
    {
 ...
 res.json({
 status: 'success',
 id: doc.idposta // kao id vraćamo generirani _id
 });
 } else {
 res.statusCode = 500;
 res.json({
...
app.get('/posts/:postId/comments', async (req, res) => {
    ...
    let cursor = await db.collection('postcomments').find(selekcija);
    let results = await cursor.toArray();
